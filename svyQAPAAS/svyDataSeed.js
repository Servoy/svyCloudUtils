 ...
 */
var DB_CACHE = { postgres: { }, mssql: { }, openedge: { } };
...
function buildSelectSQL(dbName, jsTable, additionalFilters) {
	...
	return { query: dbSQL, cntQuery: cntSql, args: queryArgs, largeDataFields: largeTxtFields, base64Fields: base64Fields };
}
	...
	for (var tableIndex in tables) {
		...
		do {
			...
			if (queryObj.base64Fields.length) {
				...
				fields.forEach(/**	@param {String} fieldName
				 @param {Number} colIndex */ function(fieldName, colIndex) {
...
function importCsvFile(dbName, tableName, file) {
	...
	if (!isNewExport) {
		...
		scopes.svyDataUtils.parseCSV(fileData, { delimiter: ',', firstRowHasColumnNames: true, textQualifier: '"' }, importData);

		//Force call parseCSV with dummy csv to workaround Rhino function arguments being kept inmem
		scopes.svyDataUtils.parseCSV('"dummy"', { delimiter: ',', firstRowHasColumnNames: true, textQualifier: '"' });
	} else {
		var csvObj = { columnNames: [], data: [] };
...
//		enableConstraints = "ALTER TABLE " + table.getQuotedSQLName() + " CHECK CONSTRAINT ALL;";
	...
	Packages.com.servoy.j2db.J2DBGlobals.getServiceProvider().getScheduledExecutor().execute(new java.lang.Runnable({run: function() {
		hasAsyncCall = true;
